...
## Поиск и источники музыки

- В MVP поиск реализован по демо-каталогу (`/search` по треку/альбому/артисту).
- Для внешних источников используйте только легальные API/каталоги с правами на стриминг.
- В Android добавлена возможность загружать треки с устройства (с запросом разрешения) и слушать их через foreground playback service в фоне.
